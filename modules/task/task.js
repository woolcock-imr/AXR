...
var site_filter_tid	=_mlist[_ids.site_filter].table_id;
//-------------------------------------
_record_type="s2";
var _task_fields='';
//-------------------------------------
...
var site_filter_and_request=function(){
    ...
    var sql="select List=@('Site_List') from [TABLE-"+site_filter_tid+"] where S2=@S1";
...
_set_req=function(){
    ...
    var sql_n="with participant as (select ID,ParticipantUID=UID from [TABLE-"+participant_tid+"]"+site_sql_where+" )";
    sql_n+=",task as (select GUID from [TABLE-"+_db_pid+"] ) ";
    sql_n+="select count(ID) from participant left join task on  GUID=ParticipantUID ";

    var sql="with participant as (select Site=S1,ParticipantUID=UID from [TABLE-"+participant_tid+"]"+site_sql_where+" )";
    sql+=",task as (select ID,PID,UID,PUID,GUID,S2,Information,DateTime,Author from [TABLE-"+_db_pid+"] ) ";
    sql+=",result as (select ID,PID,UID,PUID,GUID,S2,Site,ParticipantUID,Information,DateTime,Author,RowNum=row_number() over (order by ID DESC) from participant left join task on  GUID=ParticipantUID ) ";
    sql+="select ID,S2,UID,GUID=ParticipantUID,Task_ID=PID,Site,Information,DateTime,Author,dirty=0, valid=1 from result where RowNum between @I6 and @I7";
    _req={cmd:'query_records',sql:sql,sql_n:sql_n,s1:'"'+$('#keyword__ID').val()+'"',I:$('#I__ID').text(),page_size:$('#page_size__ID').val()}
}
//-------------------------------------
var _set_req_export=function(){
    var sql="with participant as (select Site=S1,ParticipantUID=UID,RowNum=row_number() over (order by ID DESC) from [TABLE-"+participant_tid+"]"+site_sql_where+" )";
    sql+=",task as (select ID,PID,UID,PUID,GUID,S2,Information,DateTime,Author from [TABLE-"+_db_pid+"] ) ";
    sql+="select ID,S2,UID,GUID=ParticipantUID,Task_ID=PID,Site,Information,DateTime,Author,dirty=0, valid=1 from participant left join task on  GUID=ParticipantUID order by GUID";
    _set_from_to();
